...
from src.core.skills import SkillManager
from src.ui.windows import CropGuideWindow, SkillTreeWindow, CropDetailWindow, CodeEditorWindow, NewFileModal, FileBrowserWindow
...
class GameIDE:
    def __init__(self):
        ...
        self.editor_windows = [] # Replaces legacy tab system
        
        self.undo_stack = [] 
        self.redo_stack = []
        self.aux_windows = [] # For Modals (NewFile, FileBrowser)
        ...
        for fname, fcode in spawn_files.items():
            self.editor_windows.append(CodeEditorWindow(
                self.ui_manager, fname, fcode, self
            ))
            
        # Hide all initially
        for win in self.editor_windows: win.hide()
...
"""
    ...
    def _init_ui_elements(self):
        ...
        self.btn_files = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((current_x, 10), (70, 30)),
                                                    text='FILES', manager=self.ui_manager)
        current_x += 70 + gap
        
        self.btn_new_file = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((current_x, 10), (30, 30)),
                                                    text='+', manager=self.ui_manager)
        current_x += 30 + gap
        
        self.btn_save = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((current_x, 10), (btn_w, 30)),
                                                    text='SAVE', manager=self.ui_manager)
        current_x += btn_w + gap

        self.btn_load = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((current_x, 10), (btn_w, 30)),
                                                    text='LOAD', manager=self.ui_manager)
        current_x += btn_w + gap

        self.btn_guide = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((current_x, 10), (btn_w, 30)),
                                                    text='GUIDE', manager=self.ui_manager)
        current_x += btn_w + gap

        self.btn_skills = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((current_x, 10), (btn_w, 30)),
        ...
        # 2. Editor Panel
        # 2. Console (Floating Window now? Or keep bottom panel? User asked for multi-file console effect...)
        # Actually user said "Console's multi-file ... like independent window". 
        # The Console Output itself is usually single.
        # But we previously had it inside the Editor Panel.
        # Now there is no Editor Panel.
        # We should make a Console Window too! Or attach it to main view.
        # For now, let's keep console as a persistent bottom overlay or window.
        
        # Create a Console Window
        # Create a Console Window (Simplified)
        # Using a transparent panel or just the textbox if supported. 
        # To make it "less obtrusive", we'll lower the height and remove the "System Ready" overlay text initially.
        # Create Console Output (Temporarily Disabled)
        self.console_output = None 
        # self.console_output = pygame_gui.elements.UITextBox(
        #     relative_rect=pygame.Rect((10, SCREEN_HEIGHT - 120), (SCREEN_WIDTH - 20, 100)),
        #     html_text="", # Clean start
        #     manager=self.ui_manager
        #     # No container, so it floats freely
        # )
    ...
    def draw_game_area(self):
        ...
        for y in range(self.farm.height):
            for x in range(self.farm.width):
                ...
                pygame.draw.rect(self.window, (80, 100, 120), (r_x, r_y, GRID_SIZE, GRID_SIZE), 1)
        ...
        for y in range(self.farm.height):
            for x in range(self.farm.width):
                ...
                if crop and not isinstance(crop, OccupiedSlot):
                    ...
                    img = self.visual_manager.get_asset(f"crop_{base_name}_stage{stage}")
                    ...
                    if img:
                        ...
                        if scale > 1:
                            pygame.draw.rect(self.window, (255, 215, 0), (r_x, r_y, target_size, target_size), 2)
        ...
        self.window.blit(drone_img, drone_img.get_rect(center=(d_x + GRID_SIZE//2, d_y + GRID_SIZE//2)))
        
        self.visual_manager.draw(self.window)
        self.cutscene_mgr.draw(self.window)
        
        # HUD
        if not hasattr(self, 'font_hud'): self.font_hud = pygame.font.SysFont("Consolas", 18, bold=True)
        lines = [("FARM OS V3.2", (100, 200, 255)), ("STATUS: ONLINE", (50, 255, 50))]
        for k, v in self.drone.inventory.items(): lines.append((f"  • {k.upper()}: {v}", (255, 255, 255)))
    ...
    def get_screen_coords(self, grid_x, grid_y):
        view_width = SCREEN_WIDTH - UI_PANEL_WIDTH if self.editor_visible else SCREEN_WIDTH
        start_x = (view_width - self.farm.width * GRID_SIZE) // 2
        start_y = (SCREEN_HEIGHT - self.farm.height * GRID_SIZE) // 2
    ...
    def run(self):
        ...
        # Initial Render
        # Initial Render
        # self._update_editor_display() # Removed legacy
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                ...
                try:
                    ...
                    if "container_to_scroll" not in str(e): print(f"UI Error: {e}")
                
                self.cutscene_mgr.handle_event(event)

                if event.type == pygame.KEYDOWN:
                    # Showcase Abort
                    if self.demo_active and (event.key == pygame.K_ESCAPE or event.key == pygame.K_q):
                        self.drone._stop_flag = True
                        self.editor_visible = True
                        self.editor_panel.show()
                        self.print_to_console("Showcase Cancelled.")

                    elif event.key == pygame.K_F5: self.cutscene_mgr.start_intro()
                ...
                if event.type == pygame_gui.UI_BUTTON_PRESSED:
                    ...
                    for win in self.editor_windows:
                        if event.ui_element == win.btn_run:
                            self.run_user_code(code_string=win.raw_code)
                            handled_editor = True
                            break
                        ...
                        elif hasattr(win, 'btn_stop') and event.ui_element == win.btn_stop:
                            self.drone._stop_flag = True
                            self.print_to_console(f"<font color='#FFA500'>Station: Stopped {win.filename}.</font>")
                            handled_editor = True
                            break
                    ...
                    if not handled_editor:
                        if event.ui_element == self.btn_save: 
                            # Global save
                            SaveManager.save_game(self.farm, self.drone, "") # Empty code?
                            ...
                            if not self.windows['guide'].window.alive(): self.windows['guide'] = CropGuideWindow(self.ui_manager)
                            self.windows['guide'].show()
                            
                        elif event.ui_element == self.btn_skills:
                            if not self.windows['skills'].window.alive(): self.windows['skills'] = SkillTreeWindow(self.ui_manager, self.skill_manager, self.drone)
                    ...
                    if self.windows['skills'].window.alive():
                        for btn in self.windows['skills'].buttons:
                            if event.ui_element == btn:
                                ...
                                status_str = "<font color='#00FF00'>[UNLOCKED]</font>" if node.unlocked else "<font color='#FF4444'>[LOCKED]</font>"
                                ...
                                if not node.unlocked:
                                    if self.skill_manager.unlock(btn.skill_id, self.drone.inventory):
                                        self.visual_manager.play_sound("ding")
                                        self.windows['skills'].refresh()
                                        self.cutscene_mgr.trigger("skill_unlocked")
                                        # Update text to unlocked
                                        self.windows['skills'].desc_box.set_text(detail_html.replace("LOCKED", "UNLOCKED").replace("#FF4444", "#00FF00"))
                                    else:
                            ...
                            if hasattr(event.ui_element, 'crop_cls'):
                                ...
                                if hasattr(self.windows['guide'], 'detail_window') and \
                                   self.windows['guide'].detail_window and \
                                   self.windows['guide'].detail_window.window.alive():
            ...
            try:
                ...
                if "pressed_event" not in str(e) and "held" not in str(e) and "container_to_scroll" not in str(e):
            ...
            # Draw Demo Overlay
            if self.demo_active:
                font = getattr(self, 'font_hud', None) or pygame.font.SysFont("Consolas", 24, bold=True)
                txt = font.render("ATTRACT MODE - PRESS [ESC]", True, (255, 255, 255))
                self.window.blit(txt, txt.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT - 60)))
